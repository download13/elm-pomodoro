...
Elm.Types.make = function (_elm) {
   ...
   var _op = {};
   var UserSetTimer = function (a) {
      return {ctor: "UserSetTimer",_0: a};
   };
   var TimerDelta = function (a) {
      return {ctor: "TimerDelta",_0: a};
   };
   var MarkTaskComplete = {ctor: "MarkTaskComplete"};
   var SetTaskInput = function (a) {
      return {ctor: "SetTaskInput",_0: a};
   };
   var CreateTask = {ctor: "CreateTask"};
   var Reset = {ctor: "Reset"};
   var StartOrPause = {ctor: "StartOrPause"};
   var NoOp = {ctor: "NoOp"};
   var RingingState = {ctor: "RingingState"};
   var PausedState = {ctor: "PausedState"};
   var RunningState = {ctor: "RunningState"};
   var ResetState = {ctor: "ResetState"};
   var isStatusReady = function (status) {
      return _U.eq(status,ResetState) || _U.eq(status,PausedState);
   };
...
Elm.Model.make = function (_elm) {
   ...
   var defaultBreakLength = 5 * $Time.minute;
   var defaultPomodoroLength = 20 * $Time.minute;
   var emptyModel = {status: $Types.ResetState
   ...
   return _elm.Model.values = {_op: _op
                              ,defaultPomodoroLength: defaultPomodoroLength
...
Elm.Utils.make = function (_elm) {
   ...
   $Types = Elm.Types.make(_elm);
   var _op = {};
   var clockRe = $Regex.regex("\\d+");
   var parseClock = function (value) {
      var _p0 = A2($List.map,
      function (_) {
         return _.match;
      },
      A3($Regex.find,$Regex.All,clockRe,value));
      if (_p0.ctor === "[]") {
            return $Maybe.Nothing;
         } else {
            if (_p0._1.ctor === "[]") {
                  return $Result.toMaybe($String.toInt(_p0._0));
               } else {
                  if (_p0._1._1.ctor === "[]") {
                        var _p1 = $Result.toMaybe($String.toInt(_p0._1._0));
                        if (_p1.ctor === "Nothing") {
                              return $Maybe.Nothing;
                           } else {
                              return $Maybe.Just(_p1._0 + function () {
                                 var _p2 = $Result.toMaybe($String.toInt(_p0._0));
                                 if (_p2.ctor === "Just") {
                                       return _p2._0 * 60;
                                    } else {
                                       return 0;
   ...
   var parseUserTime = function (value) {
      var _p3 = parseClock(value);
      if (_p3.ctor === "Just") {
            return $Types.UserSetTimer($Basics.toFloat(_p3._0) * $Time.second);
         } else {
            return $Types.NoOp;
         }
   };
   var onBlurValue = F2(function (address,valueToAction) {
      return A3($Html$Events.on,
      "blur",
      $Html$Events.targetValue,
      function (v) {
         return A2($Signal.message,address,valueToAction(v));
      });
   });
   var isEnter = A2($Json$Decode.customDecoder,
   $Html$Events.keyCode,
   function (code) {
      return _U.eq(code,
   ...
   var onEnterValue = F2(function (address,valueToAction) {
      return A3($Html$Events.on,
      "keydown",
      A3($Json$Decode.object2,
      F2(function (_p5,value) {    return value;}),
      isEnter,
      $Html$Events.targetValue),
      function (value) {
         return A2($Signal.message,address,valueToAction(value));
      });
   });
   var leadingZeros = F2(function (zeros,nstr) {
      var neededZeros = A2($Basics.max,
      0,
      zeros - $String.length(nstr));
      return A2($Basics._op["++"],
      A2($String.repeat,neededZeros,"0"),
      nstr);
   });
   var atLeastTwoDigits = function (nstr) {
      return A2(leadingZeros,2,nstr);
   };
   var formatTime = function (time) {
      var totalSeconds = $Basics.truncate($Time.inSeconds(time));
      var minutes = $Basics.toString(totalSeconds / 60 | 0);
      var seconds = $Basics.toString(A2($Basics._op["%"],
      totalSeconds,
      60));
      return A2($Basics._op["++"],
      atLeastTwoDigits(minutes),
      A2($Basics._op["++"],":",atLeastTwoDigits(seconds)));
   };
...
Elm.View.make = function (_elm) {
   ...
   var _op = {};
   var taskCreator = F2(function (address,model) {
      return A2($Html.input,
      _U.list([$Html$Attributes.$class("task-creator")
              ,$Html$Attributes.placeholder("Task...")
              ,A2($Utils.onEnter,address,$Types.CreateTask)
              ,A3($Html$Events.on,
              "input",
              $Html$Events.targetValue,
              function (_p0) {
                 return A2($Signal.message,address,$Types.SetTaskInput(_p0));
              })
              ,$Html$Attributes.value(model.taskInput)]),
      _U.list([]));
   });
   var taskRow = function (task) {
      return A2($Html.section,
      _U.list([$Html$Attributes.$class("task")]),
      _U.list([$Html.text(task)]));
   };
   var taskList = function (tasks) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("tasks")]),
      A2($List.map,taskRow,tasks));
   };
   var timer = F2(function (address,_p1) {
      var _p2 = _p1;
      var _p3 = _p2.status;
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("timer")]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("timer-status-type")]),
              _U.list([]))
              ,A2($Html.input,
              _U.list([$Html$Attributes.$class("timer-display")
                      ,$Html$Attributes.readonly($Basics.not($Types.isStatusReady(_p3)))
                      ,A2($Utils.onBlurValue,address,$Utils.parseUserTime)
                      ,A2($Utils.onEnterValue,address,$Utils.parseUserTime)
                      ,$Html$Attributes.value($Utils.formatTime(_p2.timeRemaining))]),
              _U.list([]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("timer-buttons")]),
              _U.list([A2($Html.button,
                      _U.list([A2($Html$Events.onClick,address,$Types.StartOrPause)
                              ,$Html$Attributes.disabled(_U.eq(_p3,$Types.RingingState))]),
                      _U.list([$Html.text($Types.isStatusReady(_p3) ? "Start" : "Pause")]))
                      ,A2($Html.button,
                      _U.list([A2($Html$Events.onClick,address,$Types.Reset)
                              ,$Html$Attributes.disabled(_U.eq(_p3,$Types.ResetState))]),
                      _U.list([$Html.text("Reset")]))
                      ,A2($Html.button,
                      _U.list([A2($Html$Events.onClick,
                              address,
                              $Types.MarkTaskComplete)
                              ,$Html$Attributes.disabled(_U.eq($List.length(_p2.tasks),0))]),
                      _U.list([$Html.text("Task Done")]))]))]));
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("pomodoro-app")]),
      _U.list([A3($Html$Lazy.lazy2,timer,address,model)
              ,A2($Html$Lazy.lazy,taskList,model.tasks)
              ,A3($Html$Lazy.lazy2,taskCreator,address,model)]));
   });
...
Elm.Storage.make = function (_elm) {
   ...
   $Types = Elm.Types.make(_elm);
   var _op = {};
   var stringToStatus = function (str) {
      var _p0 = str;
      switch (_p0)
      {case "reset": return $Types.ResetState;
         case "running": return $Types.RunningState;
         case "paused": return $Types.PausedState;
         case "ringing": return $Types.RingingState;
         default: return $Types.ResetState;}
   };
   var statusToString = function (status) {
      var _p1 = status;
      switch (_p1.ctor)
      {case "ResetState": return "reset";
         case "RunningState": return "running";
         case "PausedState": return "paused";
         default: return "ringing";}
   };
   var fromStorageModel = function (result) {
      var _p2 = result;
      if (_p2.ctor === "Nothing") {
            return $Model.emptyModel;
         } else {
            var _p3 = _p2._0;
            return {status: stringToStatus(_p3.status)
                   ,timeRemaining: _p3.timeRemaining
                   ,pomodoroLength: _p3.pomodoroLength
                   ,breakLength: _p3.breakLength
                   ,tasks: _p3.tasks
                   ,taskInput: _p3.taskInput};
         }
   };
   var toStorageModel = function (model) {
      return {status: statusToString(model.status)
   ...
   var StorageModel = F6(function (a,b,c,d,e,f) {
      return {status: a
             ,timeRemaining: b
             ,pomodoroLength: c
             ,breakLength: d
   ...
   return _elm.Storage.values = {_op: _op
                                ...
                                ,fromStorageModel: fromStorageModel
...
Elm.Pomodoro.make = function (_elm) {
   ...
   function (v) {
      return v === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v === "object" && "status" in v && "timeRemaining" in v && "pomodoroLength" in v && "breakLength" in v && "tasks" in v && "taskInput" in v ? {_: {}
                                                                                                                                                                                                                                       ,status: typeof v.status === "string" || typeof v.status === "object" && v.status instanceof String ? v.status : _U.badPort("a string",
                                                                                                                                                                                                                                       v.status)
                                                                                                                                                                                                                                       ,timeRemaining: typeof v.timeRemaining === "number" ? v.timeRemaining : _U.badPort("a number",
                                                                                                                                                                                                                                       v.timeRemaining)
                                                                                                                                                                                                                                       ,pomodoroLength: typeof v.pomodoroLength === "number" ? v.pomodoroLength : _U.badPort("a number",
                                                                                                                                                                                                                                       v.pomodoroLength)
                                                                                                                                                                                                                                       ,breakLength: typeof v.breakLength === "number" ? v.breakLength : _U.badPort("a number",
                                                                                                                                                                                                                                       v.breakLength)
                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                       v.tasks)
                                                                                                                                                                                                                                       ,taskInput: typeof v.taskInput === "string" || typeof v.taskInput === "object" && v.taskInput instanceof String ? v.taskInput : _U.badPort("a string",
                                                                                                                                                                                                                                       v.taskInput)} : _U.badPort("an object with fields `status`, `timeRemaining`, `pomodoroLength`, `breakLength`, `tasks`, `taskInput`",
      v));
   });
   var actions = $Signal.mailbox($Types.NoOp);
   var initialModel = $Storage.fromStorageModel(getStorage);
   var update = F2(function (action,model) {
      var _p0 = model;
      var status = _p0.status;
      var tasks = _p0.tasks;
      var _p1 = action;
      switch (_p1.ctor)
      {case "NoOp": return model;
         case "StartOrPause": return _U.update(model,
           {status: function () {
              var _p2 = model.status;
              switch (_p2.ctor)
              {case "ResetState": return $Types.RunningState;
                 case "RunningState": return $Types.PausedState;
                 case "PausedState": return $Types.RunningState;
                 default: return _p2;}
           }()});
         case "Reset": return _U.update(model,
           {status: $Types.ResetState
           ,timeRemaining: model.pomodoroLength});
         case "SetTaskInput": return _U.update(model,
           {taskInput: _p1._0});
         case "CreateTask": var _p3 = model;
           var taskInput = _p3.taskInput;
           var tasks = _p3.tasks;
           return _U.eq(taskInput,"") ? model : _U.update(model,
           {tasks: A2($List.append,tasks,_U.list([taskInput]))
           ,taskInput: ""});
         case "MarkTaskComplete": var _p4 = tasks;
           if (_p4.ctor === "::") {
                 return _U.update(model,{tasks: _p4._1});
              } else {
                 return model;
              }
         case "TimerDelta": if (_U.eq(status,$Types.RunningState)) {
                 var timeLeft = model.timeRemaining - _p1._0;
                 return _U.cmp(timeLeft,0) < 1 ? _U.update(model,
                 {status: $Types.RingingState
                 ,timeRemaining: 0}) : _U.update(model,
                 {timeRemaining: timeLeft});
              } else return model;
         default: return $Types.isStatusReady(status) ? _U.update(model,
           {timeRemaining: _p1._0}) : model;}
   });
   ...
   A2($Signal.merge,
   actions.signal,
   A2($Signal.map,$Types.TimerDelta,$Time.fps(5))));
   ...
   function (v) {
      return {status: v.status
             ...
             ,pomodoroLength: v.pomodoroLength
             ,breakLength: v.breakLength
             ,tasks: Elm.Native.List.make(_elm).toArray(v.tasks).map(function (v) {
                return v;
             })
             ,taskInput: v.taskInput};
   },
   A2($Signal.map,$Storage.toStorageModel,model));
   var setRinging = Elm.Native.Port.make(_elm).outboundSignal("setRinging",
   function (v) {
      return v;
   },
   A2($Signal.map,
   function (_p5) {
      var _p6 = _p5;
      return _U.eq(_p6.status,$Types.RingingState);
   },
   model));
   return _elm.Pomodoro.values = {_op: _op
                                 ,update: update
                                 ,main: main
                                 ,model: model
